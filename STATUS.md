...
- Hub root modernization: node >= 24 engines, `packageManager: pnpm@10.34.4`, `.nvmrc`, turbo `test` task with `^build` deps, root vitest projects config, `.github/workflows/ci.yml` (pnpm setup, then turbo lint/check-types/test/build).
