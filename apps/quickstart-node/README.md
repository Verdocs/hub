...
All logic is in [`index.js`](index.js). Depends on `@verdocs/js-sdk` only. Environment variables load through Node's built-in `process.loadEnvFile()`, so there is no dotenv package.
...
2. `client_credentials` grant: the API key is the identity, and no user logs in.
