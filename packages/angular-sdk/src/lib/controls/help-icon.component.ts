import { ChangeDetectionStrategy, Component, input, signal } from '@angular/core';

// aria-describedby needs a page-unique id per instance; Angular has no useId
// equivalent, so a module counter does the job.
let nextTooltipId = 0;

/**
 * Displays a simple help icon. Upon hover or focus, a tooltip will be displayed
 * with help text. Project custom icon content to replace the standard help icon,
 * the Angular take on React's icon prop:
 *
 * ```html
 * <verdocs-help-icon text="Explains the thing"><svg>...</svg></verdocs-help-icon>
 * ```
 */
@Component({
  selector: 'verdocs-help-icon',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { '[style.display]': `'inline-block'` },
  template: `
    <span class="vdocs:font-sans vdocs:relative vdocs:inline-block vdocs:opacity-30 vdocs:hover:opacity-100 vdocs:focus-within:opacity-100">
      <span
        role="img"
        aria-label="Help"
        [attr.aria-describedby]="tooltipId"
        tabindex="0"
        class="vdocs:inline-block vdocs:text-muted vdocs:outline-none"
        (mouseenter)="showing.set(true)"
        (mouseleave)="showing.set(false)"
        (focus)="showing.set(true)"
        (blur)="showing.set(false)">
        <ng-content>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="vdocs:size-6" aria-hidden="true">
            <path
              d="M11.925 18q.55 0 .938-.387.387-.388.387-.938 0-.55-.387-.925-.388-.375-.938-.375-.55 0-.925.375t-.375.925q0 .55.375.938.375.387.925.387Zm-.95-3.85h1.95q0-.8.2-1.287.2-.488 1.025-1.288.65-.625 1.025-1.213.375-.587.375-1.437 0-1.425-1.025-2.175Q13.5 6 12.1 6q-1.425 0-2.35.775t-1.275 1.85l1.775.7q.125-.45.55-.975.425-.525 1.275-.525.725 0 1.1.412.375.413.375.888 0 .475-.287.9-.288.425-.713.775-1.075.95-1.325 1.475-.25.525-.25 1.875ZM12 22.2q-2.125 0-3.988-.8-1.862-.8-3.237-2.175Q3.4 17.85 2.6 15.988 1.8 14.125 1.8 12t.8-3.988q.8-1.862 2.175-3.237Q6.15 3.4 8.012 2.6 9.875 1.8 12 1.8t3.988.8q1.862.8 3.237 2.175Q20.6 6.15 21.4 8.012q.8 1.863.8 3.988t-.8 3.988q-.8 1.862-2.175 3.237Q17.85 20.6 15.988 21.4q-1.863.8-3.988.8Zm0-2.275q3.325 0 5.625-2.3t2.3-5.625q0-3.325-2.3-5.625T12 4.075q-3.325 0-5.625 2.3T4.075 12q0 3.325 2.3 5.625t5.625 2.3ZM12 12Z" />
          </svg>
        </ng-content>
      </span>

      @if (showing()) {
        <span
          [id]="tooltipId"
          role="tooltip"
          class="vdocs:absolute vdocs:top-full vdocs:left-1/2 vdocs:-translate-x-1/2 vdocs:mt-1 vdocs:z-[10005] vdocs:min-w-[200px] vdocs:max-w-[240px] vdocs:rounded-ctl vdocs:bg-surface vdocs:px-2.5 vdocs:py-[5px] vdocs:text-xs vdocs:font-medium vdocs:text-ink vdocs:shadow-[0_0_10px_1px_#999999]">
          {{ text() }}
          <span class="vdocs:absolute vdocs:top-[-4px] vdocs:left-1/2 vdocs:-ml-1 vdocs:size-2 vdocs:rotate-45 vdocs:bg-surface"></span>
        </span>
      }
    </span>
  `,
})
export class VerdocsHelpIconComponent {
  /** Help text to display on hover/focus. */
  readonly text = input('');

  protected readonly showing = signal(false);
  protected readonly tooltipId = `verdocs-help-tooltip-${nextTooltipId++}`;
}
