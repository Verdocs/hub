...
@Component({
  ...
  host: {
    role: 'status',
    'aria-label': 'Loading',
    class: 'vdocs:block vdocs:absolute vdocs:top-1/2 vdocs:left-1/2 vdocs:-mt-3 vdocs:-ml-3 vdocs:size-6',
  },
  template: `
    <div class="vdocs:relative vdocs:size-6 vdocs:animate-spin">
      @for (dot of dots; track dot) {
        <div [class]="'vdocs:absolute vdocs:size-6 vdocs:rounded-full ' + dot"></div>
      }
    </div>
  `,
})
export class VerdocsLoaderComponent {
  protected readonly dots = DOT_CLASSES;
}
