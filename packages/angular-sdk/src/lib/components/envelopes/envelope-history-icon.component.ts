...
/**
 * The names prepareHistoryEntries assigns to envelope activity events, matching
 * the legacy web-sdk's icon keys.
 */
export type THistoryIcon = | 'account_circle' |
  'cancel' |
  'check_circle' |
  'clear' |
  'contact_email' |
...
/**
 * One envelope-history activity glyph, selected by name. The artwork is the
 * legacy web-sdk's raw SVG set, normalized to currentColor so the icons
 * inherit the sidebar's white text. A child of the envelope sidebar; not
 * useful on its own.
 */
@Component({
  selector: 'verdocs-envelope-history-icon',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { '[style.display]': `'block'` },
  template: `
    @switch (name()) {
      @case ('visibility') {
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" class="vdocs:block vdocs:size-6">
      ...
      @case ('idcard') {
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" class="vdocs:block vdocs:size-6">
          <path d="M16 10H18" />
          <path d="M16 14H18" />
...
export class VerdocsEnvelopeHistoryIconComponent {
  /** The activity icon to display. */
  readonly name = input.required<THistoryIcon>();
}
