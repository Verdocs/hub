...
import { VerdocsQuestionDialogComponent } from '../../dialogs/question-dialog.component';
...
@Component({
  ...
  template: `
    <div
      class="vdocs:fixed vdocs:inset-x-0 vdocs:bottom-0 vdocs:box-border vdocs:flex vdocs:h-12 vdocs:flex-row vdocs:items-center vdocs:justify-between vdocs:gap-2 vdocs:border-0 vdocs:border-t vdocs:border-solid vdocs:border-edge-light vdocs:bg-surface vdocs:px-6 vdocs:font-sans vdocs:text-xs vdocs:text-muted">
      @if (organization()?.powered_by_label) {
        <div [class]="'vdocs:whitespace-nowrap ' + (hasButtons() ? 'vdocs:max-[720px]:hidden' : 'vdocs:max-[540px]:hidden')">
          @if (organization()?.powered_by_url) {
            <a [href]="organization()?.powered_by_url" target="_blank" rel="noopener noreferrer" [class]="linkClasses">
              {{ organization()?.powered_by_label }}
            </a>
          } @else {
            <span>{{ organization()?.powered_by_label }}</span>
          }
        </div>
      }
      ...
      @if (hasButtons()) {
        <div
          ...
          ">
          <button type="button" [class]="buttonClasses" (click)="askingQuestion.set(true)">
          ...
          <button type="button" [class]="buttonClasses" (click)="decline.emit()">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
          ...
          <button type="button" [class]="buttonClasses" (click)="finishLater.emit()">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
      ...
      <div [class]="'vdocs:flex vdocs:flex-row vdocs:gap-2 vdocs:whitespace-nowrap ' + (hasButtons() ? 'vdocs:max-[540px]:hidden' : '')">
        @if (organization()?.terms_use_url; as termsUrl) {
          <a [href]="termsUrl" target="_blank" rel="noopener noreferrer" [class]="linkClasses">Terms of Use</a>
        }
        @if (organization()?.privacy_policy_url; as privacyUrl) {
          <a [href]="privacyUrl" target="_blank" rel="noopener noreferrer" [class]="linkClasses">Privacy Policy</a>
        }
      </div>
    </div>

    @if (askingQuestion()) {
      <verdocs-question-dialog (submit)="submitQuestion($event)" (cancel)="askingQuestion.set(false)" />
    }
  `,
})
export class VerdocsSignFooterComponent {
  /**
   ...
   */
  readonly organization = input<Partial<IOrganization> | null>(null);
  /** Once the recipient is done signing, the action buttons are hidden and only branding remains. */
  readonly isDone = input(false);

  /** Emitted with the entered text when the user asks the sender a question. The caller delivers it. */
  readonly askQuestion = output<string>();
  /** Emitted when the user declines to sign. */
  readonly decline = output<void>();
  /** Emitted when the user wants to save their progress and finish later. */
  readonly finishLater = output<void>();

  protected readonly buttonClasses = BUTTON_CLASSES;
  protected readonly linkClasses = LINK_CLASSES;

  protected readonly askingQuestion = signal(false);

  protected readonly hasButtons = computed(() => !this.isDone());

  protected readonly justButtons = computed(() => {
    const organization = this.organization();
    return !organization?.powered_by_label && !organization?.terms_use_url && !organization?.privacy_policy_url;
  });

  protected submitQuestion(question: string) {
