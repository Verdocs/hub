 ...
 * submitted) rather than stored, so it cannot be passed via the status input.
 */
...
export class VerdocsStatusIndicatorComponent {
  ...
  protected readonly resolvedStatus = computed<string>(() => {
  ...
  protected readonly display = computed(() => StatusIcons[this.resolvedStatus()] ?? PendingDisplay);

  protected readonly color = computed(() => (this.theme() === 'dark' ? this.display().dark : this.display().light));

  protected readonly message = computed(() => getStatusMessage(this.resolvedStatus()));

  protected readonly hostClasses = computed(
    () =>
      'vdocs:flex vdocs:flex-row vdocs:flex-nowrap vdocs:items-center vdocs:rounded-md vdocs:font-sans ' +
      `${this.size() === 'small' ? 'vdocs:text-base' : 'vdocs:text-lg'} ` +
      (this.theme() === 'dark' ? 'vdocs:text-white' : 'vdocs:text-ink'),
  );

  protected readonly iconClasses = computed(() =>
    (this.size() === 'small' ? 'vdocs:size-5 vdocs:mr-2 vdocs:shrink-0' : 'vdocs:size-[25px] vdocs:mr-2.5 vdocs:shrink-0'));
}
