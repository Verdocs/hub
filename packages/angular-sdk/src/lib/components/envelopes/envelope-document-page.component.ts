...
/**
 * One envelope document page, rendered as the server-side page image with a
 * field layer over it. Projected content renders into the field layer and
 * positions itself in the page's own PDF-point coordinate system: absolute
 * placement, left measured from the page's left edge and bottom measured up
 * from the page's bottom edge, exactly as field x/y are stored.
 *
 * Same approach as the template document page: the whole field layer is laid
 * out at the page's virtual size and scaled once to the rendered width, so
 * children keep PDF coordinates without knowing the scale. React's
 * onPageRendered geometry callback exists to let hosts re-place absolutely
 * positioned overlays themselves; the scaled layer makes that unnecessary
 * here, so there is no pageRendered output.
 */
@Component({
...
export class VerdocsEnvelopeDocumentPageComponent {
  /** URL of the server-rendered page image, e.g. from getEnvelopeDocumentPageDisplayUri(). Omit to render a loading placeholder. */
