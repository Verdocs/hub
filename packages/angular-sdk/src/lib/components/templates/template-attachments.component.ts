...
import { VerdocsProgressBarComponent } from '../../controls/progress-bar.component';
...
// The file-type badges keep their legacy brand colors (there are no theme
// tokens for them); the structural icons use currentColor instead.
const BADGE_COLORS: Record<Exclude<TFileKind, 'generic'>, { front: string; back: string }> = {
 ...
 * React's onAttachmentsChanged/onNext/onCancel/onSdkError callbacks are this
 * component's attachmentsChanged, next, cancel, and sdkError outputs.
 */
@Component({
  ...
  imports: [
    ...
    VerdocsFileChooserComponent,
    VerdocsLoaderComponent,
    VerdocsOkDialogComponent,
    VerdocsProgressBarComponent,
  ],
    ...
    @if (query.error()) {
      ...
      <div class="vdocs:max-w-[600px] vdocs:font-sans vdocs:text-ink">
        ...
        <div class="vdocs:mb-10">
          @for (doc of documents(); track doc.id) {
            <div class="vdocs:flex vdocs:flex-row vdocs:items-center vdocs:gap-4 vdocs:py-1 vdocs:text-muted">
              @switch (fileKind(doc.mime)) {
                @case ('generic') {
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" class="vdocs:size-6 vdocs:shrink-0">
                    <path d="M6 2c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6H6zm7 7V3.5L18.5 9H13z" />
                  </svg>
                }
                @default {
                  <svg width="19" height="24" viewBox="0 0 19 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="vdocs:shrink-0">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M0 16.2189V1.75354C0 0.78509 0.785066 0 1.75352 0H11.4037L18.6338 7.23012V17.6856H0.000116018V16.219L0 16.2189Z" [attr.fill]="badgeColors(doc.mime).front" />
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M18.6338 17.6855V22.5769C18.6338 23.3629 17.8489 23.9999 16.8804 23.9999H1.75352C0.785067 23.9999 0 23.3629 0 22.5769V17.6855H18.6337H18.6338Z" [attr.fill]="badgeColors(doc.mime).back" />
                    @switch (fileKind(doc.mime)) {
                      @case ('pdf') {
                      ...
                      @case ('jpg') {
                        <path fill-rule="evenodd" clip-rule="evenodd" [attr.d]="imageGlyph" fill="white" />
                    ...
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M11.4004 4.63363e-05H11.4036L18.6347 7.23109V13.2774C18.5839 13.3036 18.5406 13.3018 18.5058 13.2671C18.1634 12.9248 11.7261 6.93062 11.7261 6.44637C11.7261 4.3169 11.4043 0.0509979 11.4004 0V4.63363e-05Z" [attr.fill]="'url(#vdocs-badge-' + fileKind(doc.mime) + ')'" />
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M11.4004 4.63363e-05H11.4036L18.6347 7.23109H13.2259C12.7419 7.23109 12.2776 7.03876 11.9352 6.6964C11.5928 6.35404 11.4004 5.8897 11.4004 5.40555V0V4.63363e-05Z" [attr.fill]="badgeColors(doc.mime).back" />
                    <defs>
                      <linearGradient [id]="'vdocs-badge-' + fileKind(doc.mime)" x1="13.6443" y1="5.4399" x2="20.0976" y2="11.7889" gradientUnits="userSpaceOnUse">
              ...
              <div class="vdocs:relative vdocs:size-6 vdocs:shrink-0" [title]="doc.pages + ' page(s)'">
                <svg width="19" height="24" viewBox="0 0 19 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="vdocs:absolute vdocs:top-0 vdocs:left-0">
        ...
        @if (uploading()) {
          <div class="vdocs:relative vdocs:box-border vdocs:flex vdocs:h-80 vdocs:w-80 vdocs:flex-col vdocs:justify-end vdocs:border-2 vdocs:border-dashed vdocs:border-edge vdocs:bg-canvas">
            <verdocs-loader />
            <div class="vdocs:p-5">
              <verdocs-progress-bar [showPercent]="true" [percent]="progress().percent" [label]="progress().label" />
            </div>
          </div>
        } @else {
          <div class="vdocs:my-2.5 vdocs:border-2 vdocs:border-dashed vdocs:border-edge">
            <verdocs-file-chooser (selectFiles)="onSelectFiles($event)" />
          </div>
        }

        <div class="vdocs:flex vdocs:flex-row vdocs:items-center vdocs:justify-end vdocs:gap-2 vdocs:mt-4">
          <verdocs-button variant="outline" label="Cancel" size="small" [disabled]="uploading()" (click)="cancel.emit()" />
          <verdocs-button label="Next" size="small" [disabled]="!documents().length || uploading()" (click)="onNext()" />
        </div>
      </div>
      ...
      @if (showDeleteError()) {
        <verdocs-ok-dialog
          ...
          (ok)="showDeleteError.set(false)"
          (cancel)="showDeleteError.set(false)" />
      }
      ...
      @if (confirmingDelete()) {
        <verdocs-ok-dialog
          ...
          [showCancel]="true"
          (ok)="confirmDelete()"
          (cancel)="confirmingDelete.set(null)" />
      }
    }
  `,
})
export class VerdocsTemplateAttachmentsComponent {
  ...
  /** Emitted after an attachment is added or removed, with the refreshed template. */
  readonly attachmentsChanged = output<ITemplateEvent>();
  /** Emitted when the user clicks Next. */
  readonly next = output<ITemplateEvent>();
  ...
  protected readonly imageGlyph = IMAGE_GLYPH;
  protected readonly placeholders = [ 0, 1, 2 ];

  protected readonly progress = signal({ label: 'Uploading...', percent: 0 });
  protected readonly confirmingDelete = signal<ITemplateDocument | null>(null);
  protected readonly showDeleteError = signal(false);
  ...
  private readonly resolvedEndpoint = computed(() => {
    ...
    if (!resolved) {
      throw new Error('verdocs-template-attachments needs provideVerdocs() in your application providers or an explicit endpoint input');
    }

    return resolved;
  });

  protected fileKind(mime: string): TFileKind {
    return FILE_KINDS[mime] || 'generic';
  }

  protected badgeColors(mime: string) {
    const kind = this.fileKind(mime);
    return kind === 'generic' ? BADGE_COLORS.doc : BADGE_COLORS[kind];
  }

  private notifyChanged() {
    // The service awaits the detail refresh before the mutation resolves, so
    // the query already holds the refreshed template.
    const fresh = this.query.data();
    if (fresh) {
      this.attachmentsChanged.emit({ endpoint: this.resolvedEndpoint(), template: fresh });
    }
  }

  protected async onSelectFiles(files: File[]) {
    const file = files[0];
    if (!file) {
      return;
    }

    this.progress.set({ label: 'Uploading...', percent: 0 });
    this.uploading.set(true);
    try {
      await this.detailService.createDocument(this.templateId(), file, percent => this.onUploadProgress(percent), this.endpoint());
      this.notifyChanged();
    } catch (error) {
      this.sdkError.emit(toSDKError(error));
    } finally {
  ...
  private onUploadProgress(percent: number) {
    ...
    this.progress.set(percent >= 99 ? { label: 'Processing...', percent: 100 } : { label: 'Uploading...', percent });
  }

  protected requestDelete(doc: ITemplateDocument) {
    if (this.documents().length > 1) {
      this.confirmingDelete.set(doc);
    } else {
      this.showDeleteError.set(true);
    }
  }
  ...
  protected async confirmDelete() {
    ...
    try {
      await this.detailService.deleteDocument(this.templateId(), doc.id, this.endpoint());
      this.notifyChanged();
    } catch (error) {
      this.sdkError.emit(toSDKError(error));
    } finally {
  ...
  protected onNext() {
    ...
    if (template) {
      this.next.emit({ endpoint: this.resolvedEndpoint(), template });
    }
  }
}
