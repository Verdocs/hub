 ...
 * callback) so the host can run the adopt-initials dialog. Once initialed
 * (initial-url set) or done, the adopted image is drawn sized to the field
 * box.
 *
 * The legacy component fetched the initials blob by ID itself. Here the host
 * resolves the image and passes a URL; the sign embed will own that lookup,
 * along with the legacy Edit/Clear menu on a completed field. Builder-only
 * affordances (drag, resize, the settings popover) are omitted per
 * docs/PORTING.md.
 *
 * @fires vdocs-begin-signing - Fired when the user clicks the unsigned field to start the adopt-initials flow.
 */
export class VdocsFieldInitial extends VdocsElement implements IFieldBaseProperties {
  static override properties = {
    field: { attribute: false },
    initialUrl: { type: String, attribute: 'initial-url' },
  ...
  override updated(changed: PropertyValues<this>) {
    ...
    syncFieldClasses(this, [
      ...
      // Filled fields drop their border and background so only the image
  ...
  override render() {
    ...
    if (this.done) {
      return this.initialUrl ? html`<img class=${IMAGE_CLASSES} src=${this.initialUrl} alt="Initial" />` : nothing;
    }
      ...
      ${this.initialUrl ?
        html`
          <div class="vdocs:relative vdocs:size-full${this.disabled ? ' vdocs:opacity-50 vdocs:pointer-events-none' : ''}">
            <img class=${IMAGE_CLASSES} src=${this.initialUrl} alt="Initial" />
          </div>` :
        html`
          <button
            type="button"
...
register('vdocs-field-initial', VdocsFieldInitial);

declare global {
  interface HTMLElementTagNameMap {
    'vdocs-field-initial': VdocsFieldInitial;
  }
}
