...
export type TStatusIndicatorTheme = 'light' | 'dark';
export type TStatusIndicatorSize = 'small' | 'normal';
...
const acceptedIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
...
const canceledIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
...
const completedIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
...
const declinedIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
...
const pendingIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
    <path d="M7.33333 12H7.345M12 12H12.0117M16.6667 12H16.6783M22.5 12C22.5 13.3789 22.2284 14.7443 21.7007 16.0182C21.1731 17.2921 20.3996 18.4496 19.4246 19.4246C18.4496 20.3996 17.2921 21.1731 16.0182 21.7007C14.7443 22.2284 13.3789 22.5 12 22.5C10.6211 22.5 9.25574 22.2284 7.98182 21.7007C6.70791 21.1731 5.55039 20.3996 4.57538 19.4246C3.60036 18.4496 2.82694 17.2921 2.29926 16.0182C1.77159 14.7443 1.5 13.3789 1.5 12C1.5 9.21523 2.60625 6.54451 4.57538 4.57538C6.54451 2.60625 9.21523 1.5 12 1.5C14.7848 1.5 17.4555 2.60625 19.4246 4.57538C21.3938 6.54451 22.5 9.21523 22.5 12Z" stroke=${color} stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
  </svg>`;

const signedIcon = (color: string, className: string): SVGTemplateResult => svg`
  <svg viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" class=${className} aria-hidden="true">
    <circle cx="12.5" cy="12.5" r="11.5" stroke=${color} stroke-width="2" />
 ...
 * status. The status may be passed directly (the `status` attribute), or a
 * whole envelope may be assigned to the `envelope` property to display its
 * overall status. Purely presentational; fires no events.
 *
 * ```html
 * <vdocs-status-indicator status="complete" theme="dark"></vdocs-status-indicator>
 * ```
 */
export class VdocsStatusIndicator extends VdocsElement {
  ...
  declare status?: TEnvelopeStatus | TRecipientStatus | 'accepted';
  /** The envelope to display status for. Ignored if `status` is set. Property-only. */
  declare envelope?: IEnvelope;
  /** Light suits light backgrounds (colored icons and dark text), dark inverts. */
  declare theme: TStatusIndicatorTheme;
  /** The small variant suits densely populated views such as table rows. */
  declare size: TStatusIndicatorSize;
  ...
  override connectedCallback() {
    ...
    // A block box so a fixed-width column class on the host (the list sets
    // w-[125px]) takes effect; the inner row lays the icon and label out.
  ...
  private resolveStatus(): string {
    ...
    let resolved: string = this.envelope?.status ?? 'pending';
    ...
    if (recipients) {
      const submitted = recipients.filter(r => r.status === 'submitted');
      if (submitted.length > 0 && submitted.length !== recipients.length) {
        resolved = 'some-signed';
      }
    }
  ...
  override render() {
    ...
    const color = this.theme === 'dark' ? dark : light;
    const iconClass = this.size === 'small' ? 'vdocs:size-5 vdocs:mr-2 vdocs:shrink-0' : 'vdocs:size-[25px] vdocs:mr-2.5 vdocs:shrink-0';

    return html`
      <div
        class="vdocs:flex vdocs:flex-row vdocs:flex-nowrap vdocs:items-center vdocs:rounded-md vdocs:font-sans ${this.size === 'small' ?
          'vdocs:text-base' :
          'vdocs:text-lg'} ${this.theme === 'dark' ? 'vdocs:text-white' : 'vdocs:text-ink'}">
        ${icon(color, iconClass)}
        <span class="vdocs:whitespace-nowrap">${getStatusMessage(resolvedStatus)}</span>
      </div>`;
  }
}

register('vdocs-status-indicator', VdocsStatusIndicator);

declare global {
  interface HTMLElementTagNameMap {
    'vdocs-status-indicator': VdocsStatusIndicator;
  }
}
