...
import type { IDialogSubmitDetail } from '../dialogs/dialog-events.js';
import { VdocsElement } from '../base/vdocs-element.js';
import { register } from '../base/register.js';
import '../dialogs/vdocs-question-dialog.js';

const questionIcon = (): TemplateResult => html`
...
const declineIcon = (): TemplateResult => html`
  <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
...
const saveIcon = (): TemplateResult => html`
  <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
...
const BUTTON_CLASSES = 'vdocs:flex vdocs:cursor-pointer vdocs:flex-row vdocs:items-center vdocs:gap-1 vdocs:border-none vdocs:bg-transparent ' +
 ...
 * document being signed. Offers the recipient convenience actions (ask the
 * sender a question, decline, finish later) and displays the sending
 * organization's white-label branding.
 *
 * @fires vdocs-ask-question - Fired with the entered question text when the user submits the ask-a-question dialog.
 * @fires vdocs-decline - Fired when the user declines to sign.
 * @fires vdocs-finish-later - Fired when the user wants to save their progress and finish later.
 */
export class VdocsSignFooter extends VdocsElement {
  static override properties = {
    organization: { attribute: false },
    isDone: { type: Boolean, attribute: 'is-done' },
    askingQuestion: { state: true },
  };

  /** The envelope's organization, used for white-label branding. Property-only. */
  declare organization?: Partial<IOrganization> | null;
  /** Once the recipient is done signing, the action buttons are hidden and only branding remains. */
  declare isDone: boolean;

  private declare askingQuestion: boolean;
  ...
  private handleSubmitQuestion(e: CustomEvent<IDialogSubmitDetail | undefined>) {
    this.askingQuestion = false;
    this.emit('vdocs-ask-question', e.detail?.value ?? '');
  }
    ...
    return html`
      <div class="vdocs:fixed vdocs:inset-x-0 vdocs:bottom-0 vdocs:box-border vdocs:flex vdocs:h-12 vdocs:flex-row vdocs:items-center vdocs:justify-between vdocs:gap-2 vdocs:border-0 vdocs:border-t vdocs:border-solid vdocs:border-edge-light vdocs:bg-surface vdocs:px-6 vdocs:font-sans vdocs:text-xs vdocs:text-muted">
        ${organization?.powered_by_label ?
          html`
            <div class="vdocs:whitespace-nowrap ${hasButtons ? 'vdocs:max-[720px]:hidden' : 'vdocs:max-[540px]:hidden'}">
              ${organization.powered_by_url ?
                html`<a href=${organization.powered_by_url} target="_blank" rel="noopener noreferrer" class=${LINK_CLASSES}>${organization.powered_by_label}</a>` :
                html`<span>${organization.powered_by_label}</span>`}
            </div>` :
          nothing}

        ${hasButtons ?
          html`
            <div class="vdocs:flex vdocs:flex-row vdocs:items-center vdocs:gap-4 vdocs:whitespace-nowrap ${justButtons ?
              'vdocs:flex-1 vdocs:justify-center' :
              'vdocs:max-[540px]:flex-1 vdocs:max-[540px]:justify-center'}">
              <button type="button" class=${BUTTON_CLASSES} @click=${() => {
                this.askingQuestion = true;
              }}>
                ${questionIcon()}Ask Sender a Question
              </button>
              <button type="button" class=${BUTTON_CLASSES} @click=${() => this.emit('vdocs-decline')}>
                ${declineIcon()}Decline Signing
              </button>
              <button type="button" class=${BUTTON_CLASSES} @click=${() => this.emit('vdocs-finish-later')}>
                ${saveIcon()}Finish Later
              </button>
            </div>` :
          nothing}
        ...
        <div class="vdocs:flex vdocs:flex-row vdocs:gap-2 vdocs:whitespace-nowrap ${hasButtons ? 'vdocs:max-[540px]:hidden' : ''}">
          ${organization?.terms_use_url ?
            html`<a href=${organization.terms_use_url} target="_blank" rel="noopener noreferrer" class=${LINK_CLASSES}>Terms of Use</a>` :
            nothing}
          ${organization?.privacy_policy_url ?
            html`<a href=${organization.privacy_policy_url} target="_blank" rel="noopener noreferrer" class=${LINK_CLASSES}>Privacy Policy</a>` :
            nothing}
        </div>
      </div>
      ...
      ${this.askingQuestion ?
        html`
          <vdocs-question-dialog
            @vdocs-submit=${this.handleSubmitQuestion}
            @vdocs-cancel=${() => {
              this.askingQuestion = false;
            }}></vdocs-question-dialog>` :
        nothing}`;
...
register('vdocs-sign-footer', VdocsSignFooter);

// vdocs-decline is already declared by the dialogs (dialog-events.ts) with the
// same CustomEvent<undefined> payload, so we do not re-declare it here.
declare global {
  interface HTMLElementTagNameMap {
    'vdocs-sign-footer': VdocsSignFooter;
  }

  interface GlobalEventHandlersEventMap {
    'vdocs-ask-question': CustomEvent<string>;
    'vdocs-finish-later': CustomEvent<undefined>;
  }
}
