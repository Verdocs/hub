 ...
 * ancestor. There are currently no configuration options for this control, and
 * it mirrors the React SDK Loader with no props, events, or slots.
 */
...
<template>
  <div
    role="status"
    aria-label="Loading"
    class="vdocs:absolute vdocs:top-1/2 vdocs:left-1/2 vdocs:-mt-3 vdocs:-ml-3 vdocs:size-6"
  >
    <div class="vdocs:relative vdocs:size-6 vdocs:animate-spin">
      <div
        v-for="dot in DOT_CLASSES"
        :key="dot"
        :class="['vdocs:absolute vdocs:size-6 vdocs:rounded-full', dot]"
      />
    </div>
  </div>
</template>
