...
export interface VerdocsTemplateAttachmentsProps {
...
import VerdocsProgressBar from '../../controls/VerdocsProgressBar.vue';
...
const { endpoint, templateId } = defineProps<VerdocsTemplateAttachmentsProps>();

const emit = defineEmits<{
  /** React onAttachmentsChanged: fired after an attachment is added or removed, with the refreshed template. */
  attachmentsChanged: [event: ITemplateEvent];
  /** React onNext: fired when the user clicks Next. */
  next: [event: ITemplateEvent];
...
// The file-type badges keep their brand colors: there are no theme tokens for
// them, matching the React port's note. The JPG and PNG badges share the same
// image glyph, only the badge colors differ.
const FILE_COLORS: Record<string, { front: string; back: string }> = {
  pdf: { front: '#EB0005', back: '#C20004' },
  jpg: { front: '#FA8D00', back: '#FF7000' },
  png: { front: '#00C783', back: '#00AF73' },
  doc: { front: '#33A9FE', back: '#2585CB' },
};
...
const progress = ref({ label: 'Uploading...', percent: 0 });
const confirmingDelete = ref<ITemplateDocument | null>(null);
...
const handleUploadProgress = (percent: number) => {
  ...
  progress.value = percent >= 99 ? { label: 'Processing...', percent: 100 } : { label: 'Uploading...', percent };
};
...
const { mutate: removeDocument, isPending: deletingDocument } = useMutation({
...
const notifyChanged = () => {
  ...
  if (fresh) {
    emit('attachmentsChanged', { endpoint: resolvedEndpoint, template: fresh });
  }
};
...
const handleSelectFiles = (files: File[]) => {
  ...
  progress.value = { label: 'Uploading...', percent: 0 };
  uploadDocument(file, { onSuccess: notifyChanged, onError: reportError });
};

const handleDelete = (doc: ITemplateDocument) => {
  if (documents.value.length > 1) {
    confirmingDelete.value = doc;
  } else {
...
const handleConfirmDelete = () => {
  ...
  const documentId = confirmingDelete.value.id;
  confirmingDelete.value = null;
  removeDocument(documentId, { onSuccess: notifyChanged, onError: reportError });
};

const handleNext = () => {
  if (template.value) {
    emit('next', { endpoint: resolvedEndpoint, template: template.value });
  }
};
</script>
...
<template>
  ...
  <div
    v-else
    class="vdocs:max-w-[600px] vdocs:font-sans vdocs:text-ink"
  >
    <h5 class="vdocs:text-base vdocs:font-bold vdocs:text-muted vdocs:m-0 vdocs:mb-2.5">
      Existing Attachments
    </h5>

    <div class="vdocs:mb-10">
      <div
        v-for="doc in documents"
        :key="doc.id"
        class="vdocs:flex vdocs:flex-row vdocs:items-center vdocs:gap-4 vdocs:py-1 vdocs:text-muted"
      >
        <svg
          v-if="fileKind(doc.mime) === 'generic'"
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden="true"
          class="vdocs:size-6 vdocs:shrink-0"
        >
        ...
        >
          ...
          <path
            ...
            :fill="FILE_COLORS[fileKind(doc.mime)]?.back"
          />

          <template v-if="fileKind(doc.mime) === 'pdf'">
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
            ...
            />
            <path
              d="M11.8496 22.5143V18.9238H14.0495V19.711H12.8037V20.3339H13.9783V21.1039H12.8037V22.5143H11.8496V22.5143Z"
              fill="white"
            />
          </template>

          <template v-else-if="fileKind(doc.mime) === 'jpg'">
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              :d="IMAGE_GLYPH"
          ...
          <template v-else-if="fileKind(doc.mime) === 'png'">
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              :d="IMAGE_GLYPH"
        ...
        <div
          :title="doc.name"
          class="vdocs:flex-1 vdocs:overflow-hidden vdocs:whitespace-nowrap vdocs:text-ellipsis vdocs:border-b vdocs:border-dotted vdocs:border-edge-light"
        >
          {{ doc.name }}
        </div>

        <div
          class="vdocs:relative vdocs:size-6 vdocs:shrink-0"
          :title="`${doc.pages} page(s)`"
        >
          <svg
            width="19"
            height="24"
    ...
    <div
      v-if="uploading"
      class="vdocs:relative vdocs:box-border vdocs:flex vdocs:h-80 vdocs:w-80 vdocs:flex-col vdocs:justify-end vdocs:border-2 vdocs:border-dashed vdocs:border-edge vdocs:bg-canvas"
    >
      <VerdocsLoader />
      <div class="vdocs:p-5">
        <VerdocsProgressBar
    ...
    <div
      v-else
      class="vdocs:my-2.5 vdocs:border-2 vdocs:border-dashed vdocs:border-edge"
    >
      <VerdocsFileChooser @select-files="handleSelectFiles" />
    </div>
