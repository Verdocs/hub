...
export interface VerdocsStatusIndicatorProps {
...
type TStatusArtwork = 'accepted' | 'canceled' | 'complete' | 'declined' | 'in-progress' | 'opened' | 'pending' | 'signed' | 'some-signed';

interface IStatusDisplay {
  artwork: TStatusArtwork;
  light: string;
  dark: string;
}

const PendingDisplay: IStatusDisplay = { artwork: 'pending', light: '#999999', dark: '#cccccc' };

// The icon artwork and per-theme colors for each status, ported 1:1 from the
// legacy web-sdk indicator. Submitted shares the completed artwork, and invited
// shares in-progress, exactly as the legacy component mapped them.
const StatusIcons: Record<string, IStatusDisplay> = {
  accepted: { artwork: 'accepted', light: ACCENT, dark: WHITE },
  canceled: { artwork: 'canceled', light: '#9095b3', dark: '#9095b3' },
  complete: { artwork: 'complete', light: '#50be80', dark: '#63c58e' },
  declined: { artwork: 'declined', light: '#ff0000', dark: '#f95353' },
  'in progress': { artwork: 'in-progress', light: ACCENT, dark: WHITE },
  invited: { artwork: 'in-progress', light: ACCENT, dark: WHITE },
  opened: { artwork: 'opened', light: ACCENT, dark: WHITE },
  pending: PendingDisplay,
  signed: { artwork: 'signed', light: ACCENT, dark: WHITE },
  'some-signed': { artwork: 'some-signed', light: ACCENT, dark: '#63c58e' },
  submitted: { artwork: 'complete', light: '#50be80', dark: '#63c58e' },
};
...
const { status, envelope, theme = 'light', size = 'normal' } = defineProps<VerdocsStatusIndicatorProps>();
...
const resolvedStatus = computed<string>(() => {
  if (!status && envelope?.recipients) {
    const submitted = envelope.recipients.filter(recipient => recipient.status === 'submitted');
    if (submitted.length > 0 && submitted.length !== envelope.recipients.length) {
      return 'some-signed';
    }
  }

  return status || envelope?.status || 'pending';
});

const display = computed(() => StatusIcons[resolvedStatus.value] ?? PendingDisplay);
const color = computed(() => (theme === 'dark' ? display.value.dark : display.value.light));
const message = computed(() => getStatusMessage(resolvedStatus.value));

const rootClasses = computed(() => [
  'vdocs:flex vdocs:flex-row vdocs:flex-nowrap vdocs:items-center vdocs:rounded-md vdocs:font-sans',
  size === 'small' ? 'vdocs:text-base' : 'vdocs:text-lg',
  theme === 'dark' ? 'vdocs:text-white' : 'vdocs:text-ink',
]);

const iconClasses = computed(() =>
  (size === 'small' ? 'vdocs:size-5 vdocs:mr-2 vdocs:shrink-0' : 'vdocs:size-[25px] vdocs:mr-2.5 vdocs:shrink-0'));
</script>

<template>
  <div :class="rootClasses">
    <svg
      v-if="display.artwork === 'accepted'"
      viewBox="0 0 25 25"
      fill="none"
    ...
    <svg
      v-else-if="display.artwork === 'opened'"
      viewBox="0 0 25 25"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      :class="iconClasses"
    ...
    >
      <path
        d="M7.33333 12H7.345M12 12H12.0117M16.6667 12H16.6783M22.5 12C22.5 13.3789 22.2284 14.7443 21.7007 16.0182C21.1731 17.2921 20.3996 18.4496 19.4246 19.4246C18.4496 20.3996 17.2921 21.1731 16.0182 21.7007C14.7443 22.2284 13.3789 22.5 12 22.5C10.6211 22.5 9.25574 22.2284 7.98182 21.7007C6.70791 21.1731 5.55039 20.3996 4.57538 19.4246C3.60036 18.4496 2.82694 17.2921 2.29926 16.0182C1.77159 14.7443 1.5 13.3789 1.5 12C1.5 9.21523 2.60625 6.54451 4.57538 4.57538C6.54451 2.60625 9.21523 1.5 12 1.5C14.7848 1.5 17.4555 2.60625 19.4246 4.57538C21.3938 6.54451 22.5 9.21523 22.5 12Z"
        :stroke="color"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
    ...
    <span class="vdocs:whitespace-nowrap">
      {{ message }}
    </span>
  </div>
</template>
