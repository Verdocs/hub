...
export interface VerdocsEnvelopeHistoryIconProps {
  /** Which activity glyph to render. */
  name: THistoryIcon;
}
</script>

<script setup lang="ts">
defineProps<VerdocsEnvelopeHistoryIconProps>();
</script>

<template>
  <!-- One envelope-history activity glyph, selected by name. The artwork is the legacy
       web-sdk's raw SVG set, normalized to currentColor so the icons inherit the sidebar's
       white text. A child of the envelope sidebar; not useful on its own. -->
  <svg
  ...
  <svg
    v-else-if="name === 'mail' || name === 'contact_mail'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    fill="currentColor"
    aria-hidden="true"
  ...
  <svg
    v-else-if="name === 'create' || name === 'perm_identity'"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    fill="currentColor"
    aria-hidden="true"
